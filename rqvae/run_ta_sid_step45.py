"""
run_ta_sid_step45.py — Resume TA-SID from Step 4 (code gen) and Step 5 (T5)

Skips Steps 1-3 if checkpoints already exist. Run one dataset at a time.

Usage:
    python rqvae/run_ta_sid_step45.py --dataset Sport
    python rqvae/run_ta_sid_step45.py --dataset Toys
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="TA-SID Step 4+5 resume")
...
# ============================================================
# Find the best RQ-VAE checkpoint across ALL training runs
# ============================================================
ckpt_base = os.path.join(project_root, f"rqvae/ckpt/{ds}_TA_SID")
log(f"Looking for RQ-VAE checkpoint in {ckpt_base}")

if not os.path.exists(ckpt_base):
    log(f"ERROR: {ckpt_base} not found. Run Step 3 (RQ-VAE train) first!")
    sys.exit(1)

best_dir, best_collision_path, best_collision_val = find_best_checkpoint(ckpt_base)

if best_dir is None:
    log(f"ERROR: No valid checkpoint subdirectories in {ckpt_base}")
    sys.exit(1)
...
if os.path.exists(os.path.join(project_root, code_output)):
    log(f"Codes already exist at {code_output}, skipping generation")
else:
    gen_code_content = f'''"""
...
if os.path.exists(os.path.join(project_root, t5_save)):
    log(f"T5 model already exists at {t5_save}, skipping")
else:
    step5_cmd = (
...
log(f"🎉 TA-SID Step 4+5 complete for {ds}!")
log(f"   Codes: data/{ds}/{ds}_ta_sid.npy")
log(f"   Model: {t5_save}")
log(f"   Log:   {t5_log}")
